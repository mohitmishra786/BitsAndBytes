...
// Create aligned matrix with error handling
double* create_aligned_matrix() {
    double* matrix = NULL;
    if (posix_memalign((void**)&matrix, ALIGN_TO, N * N * sizeof(double)) != 0) {
...
void matrix_multiply_optimized(const double* A, const double* B, double* C) {
    ...
    // Blocked matrix multiplication with AVX and OpenMP
                    ...
                    for (int kk = k; kk < min(k + BLOCK_SIZE, N); kk++) {
                        ...
                        for (int jj = j; jj < min(j + BLOCK_SIZE, N); jj += 4) {
                            if (jj + 4 <= N) {
                                ...
                                c = _mm256_fmadd_pd(a, b, c);  // Using FMA for better performance
